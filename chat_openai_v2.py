...
st.set_page_config(
    page_title="Chat GenAI - An Intelligent Assistant",
    page_icon="data/samsunglife.png",
    layout="wide",
...
def load_updates():
    ...
    try:
        with open("data/streamlit_updates.json", "r", encoding="UTF8") as f:
            return json.load(f)
...
def get_streamlit_api_code_version():
    """
    Get the current  API code version from the Streamlit API documentation.
    ...
    try:
        ...
        logging.error(f"Error connecting to the API documentation: {str(e)}")
    return None

def display_streamlit_updates():
    """Display the latest updates """
    with st.expander("Update Announcement", expanded=False):
        st.markdown("For more details on this version, check out the [Confluence](https://www.aaa.aa).")
...
def initialize_conversation():
    ...
    """
    assistant_message = "Hello! I am genAI. How can I assist youD?"
...
def on_chat_submit(chat_input, latest_updates):
    ...
    Returns:
    - None: Updates the chat history in aa's session state.
    """
...
def main():
    ...
    img_path = "data/samsunglife.png"
    img_base64 = img_to_base64(img_path)
    if img_base64:
        st.sidebar.markdown(
            #f'<img src="data:image/png;base64,{img_base64}" class="cover-glow">',
            f'<img src="data:image/png;base64,{img_base64}" class="">',
            unsafe_allow_html=True,
        )
    ...
    mode = st.sidebar.radio("Select Mode:", options=["Latest Updates", "Chat with GenAI"], index=1)

    st.sidebar.markdown("---")

    # Display basic interactions
    show_basic_info = st.sidebar.checkbox("Show Basic Infomation", value=True)
    if show_basic_info:
        st.sidebar.markdown("""
        ### Basic Information
        - **Ask About ㅁㅁㅁ**: Type your questions about every issues.
        - **aaa**: Use keywords like 'code example', 'syntax', or 'how-to' to get relevant code snippets.
        - **aaa**: Switch to 'Updates' mode to browse the latest Streamlit updates in detail.
        """)

    # Display advanced interactions
    show_advanced_info = st.sidebar.checkbox("Show Advanced Information", value=False)
    if show_advanced_info:
        st.sidebar.markdown("""
        ### Advanced Information
        - **1.aaa**: Use keywords like ** app**, **create app** to get a basic app code.
        - **2.bbb**: Ask for ** explanation**, **walk me through the code** to understand the underlying logic of  code snippets.
        - **3.ccc**: Use **analyze my project**, **technical feedback** to get insights and recommendations on your current  project.
        - **4.ddd**: Use **debug this**, **fix this error** to get help with troubleshooting issues in your  app.
        """)

    st.sidebar.markdown("---")

    # Load and display image with glowing effect
    #img_path = "data/idea_01.png"
    #img_base64 = img_to_base64(img_path)
    #if img_base64:
    #    st.sidebar.markdown(
    #        f'<img src="data:image/png;base64,{img_base64}" class="cover-glow">',
    ...
    if mode == "Chat with GenAI":
        chat_input = st.chat_input("Ask me about ???")
        if chat_input:
        ...
        for message in st.session_state.history[-NUMBER_OF_MESSAGES_TO_DISPLAY:]:
            role = message["role"]
            avatar_image = "data/ai_robot.png" if role == "assistant" else "data/question_01.jpg" if role == "user" else None
